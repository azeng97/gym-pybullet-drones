...
class VehicleExampleRunningCost(NamedTuple):
    ...
    def __call__(self, state, control, step):
        ...
        if True:
            ...
            target_pose = 1 * ((z - target_z) ** 2) + 0.001 * (
                    (x - target_x) ** 2 + (y - target_y) ** 2 + psi_e)  # + 100*(theta - jnp.pi/8)**2
            # print(theta)
...
class VehicleExampleTerminalCost(NamedTuple):
    ...
    def __call__(self, state):
        ...
        if True:
            # Original (100x gain)
            psi_e = (psi - target_psi) ** 2
            return 100 * ((z - target_z) ** 2 + (x - target_x) ** 2 + (
                    y - target_y) ** 2 + vz ** 2 + phi ** 2 + theta ** 2 + p ** 2 + q ** 2 + r ** 2 + psi_e)
        else:
            # krt
